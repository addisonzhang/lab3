...
let point_recd_to_pair (p : point_recd) : point_pair = (p.x, p.y) ;;
...
let transcript (enrollments : enrollment list)
               (student : int)
             : enrollment list =
  filter (fun en -> en.id = student) enrollments ;;
...
let ids (enrollments: enrollment list) : int list =
  sort_uniq (compare) (map (fun en -> en.id) enrollments);;
...
let names (enrollments : enrollment list) : string list =
  sort_uniq (compare) (List.map (fun { name; _ } -> name) enrollments) ;;
...
let rec partition (arg: 'a -> bool) (lst: 'a list) : ('a list * 'a list) =
  match lst with 
  | [] -> ([], [])
  | hd :: tl -> 
    let (l1, l2) = partition arg tl in 
      if arg hd then (hd::l1, l2) else (l1, hd::l2) ;;
...
let apply (func: 'a -> 'b) (arg: 'a) : 'b = func arg ;;
